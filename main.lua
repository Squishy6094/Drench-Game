...
GAME_MODE_DATA = {
    ...
    [GAME_MODE_DICE] = {
        ...
        desc =
        "Ready to test your luck? Hit a player to have a 5% chance of killing them. Each failed hit will increase your odds by 10%, and getting hit by another player will increase your odds by 5%. Be the last one standing to win!",
        descElim =
        "Ready to test your luck? Hit a player to have a 5% chance of killing them. Each failed hit will increase your odds by 10%, and getting hit by another player will increase your odds by 5%. Who will survive?",
        level = -1,
        interact = PLAYER_INTERACTIONS_PVP, -- so invulnerability frames exist
        kbStrength = 25,
...
function mario_update(m)
    ...
    elseif gGlobalSyncTable.gameState == GAME_STATE_GAME_END then
        ...
        elseif gGlobalSyncTable.gameState == GAME_STATE_ACTIVE then
            ...
            gNametagsSettings.showHealth = gData.showHealth or (gGlobalSyncTable.gameMode == GAME_MODE_LIGHTS_OUT and sMario.spectator) or false
...
function on_pvp_attack(attacker, victim, interaction)
    ...
    elseif gGlobalSyncTable.gameMode == GAME_MODE_DICE then -- dice calculations
        local sVictim = gPlayerSyncTable[victim.playerIndex]
        local sAttacker = gPlayerSyncTable[attacker.playerIndex]
        if sVictim.team == 0 or sAttacker.team ~= sVictim.team then
            local dieMax = 20
            ...
            if roll <= chance then
                ...
                chance = math.min(sVictim.roundScore + 1, dieMax)
...
function on_nametags_render(index)
    ...
    if index ~= 0 and gGlobalSyncTable.gameMode == GAME_MODE_LIGHTS_OUT and gMarioStates[0].action ~= ACT_SPECTATE then
        return ""
    elseif gGlobalSyncTable.gameMode == GAME_MODE_DICE then
        local dieMax = 20
...
function on_packet_rejoin(data, self)
    ...
    if gGlobalSyncTable.miniGameNum == data.leftMiniGame and gGlobalSyncTable.gameState ~= GAME_STATE_SCORES then
        ...
        if gGlobalSyncTable.round == data.leftRound and data.eliminated ~= nil then
            ...
            -- if we rejoin, we automatically get a bomb
            if (not sMario.eliminated) and gGlobalSyncTable.gameMode == GAME_MODE_BOMB_TAG then
